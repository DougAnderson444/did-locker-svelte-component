<script>
  ...
  // import { DEFAULT_DID_DOC_TLD, ROOT_CID_PEM } from "../../js/constants.js";
  ...
  //   if ($ipfsNode) {
  //     $dnsLink = await getDNSLinkFromName(`${$username}.${DEFAULT_DID_DOC_TLD}`);
...
<!-- {#if $username}
  <ul>
    <li>
      Username
      <b>{$username}.{DEFAULT_DID_DOC_TLD}</b>
    </li>
    <li>
      DNSLink:
